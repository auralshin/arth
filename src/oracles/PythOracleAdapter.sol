...
contract PythOracleAdapter {
    ...
    event PriceUpdated(bytes32 indexed feedId, uint256 price, uint256 confidence, uint64 publishTime);
    ...
    /**
     * @notice Read a feed, scaled to 1e18 with confidence and freshness checks
     * @param id Pyth price feed ID
     ...
     */
    function readPriceX18(bytes32 id)
        public view
        returns (uint256 pxX18, uint256 confX18, uint256 publishTime)
    {
        ...
        if (confX18 * 10000 > pxX18 * maxConfidenceBps) {
     ...
     * @return publishTime Unix timestamp of price publication
     */
    function readPriceX18Unsafe(bytes32 id)
        external view
        returns (uint256 pxX18, uint256 confX18, uint256 publishTime)
    {
        if (!supported[id]) revert UnsupportedFeed(id);
     ...
     */
    function checkFeed(bytes32 id) external view returns (bool isSupported, bool isFresh) {
        isSupported = supported[id];
        if (!isSupported) return (false, false);
        
        try PYTH.getPriceNoOlderThan(id, maxAge) returns (PythStructs.Price memory) {
            isFresh = true;
        } catch {
            isFresh = false;
        }
